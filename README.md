...
支持格式：`jpg`、`jpeg`、`png`、`webp`、`gif`、`avif`。默认单文件最大 10MB，每次最多上传 20 张，可用环境变量调整。上传时会读取真实文件头，扩展名写错但真实类型安全时，会按真实格式自动保存，例如真实 WebP 会保存为 `.webp`。
...
- 上传后自动重命名为 `时间戳-随机字符串.扩展名`，扩展名来自服务端识别到的真实图片类型。
- 删除图片前浏览器会弹出确认框。
